...
async function requireAdminAuth(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Check 1: Custom claims on user token
    const claims = user.claims || {};
    const tokenRole = String(claims.role || claims.staffRole || '').toLowerCase();
    const tokenIsAdmin = claims.admin === true || claims.isAdmin === true;
    if (tokenIsAdmin || ['superadmin', 'admin', 'clinical_lead', 'moderator', 'support'].includes(tokenRole)) {
      return { user, db, isSuperadmin: tokenRole === 'superadmin' || tokenRole === 'admin' || tokenIsAdmin };
    }

    // Check 2: Dedicated admin document in /admins/{uid}
    const adminDoc = await db.collection('admins').doc(user.uid).get();
    if (adminDoc.exists) {
      const adminData = adminDoc.data() || {};
      if (adminData.isActive !== false) {
        const isSuper = adminData.role === 'superadmin' || adminData.role === 'admin' || !adminData.role;
        return { user, db, isSuperadmin: isSuper };
      }
    }

    // Check 3: Role / isAdmin flag on user record in /users/{uid}
    const userDoc = await db.collection('users').doc(user.uid).get();
    if (userDoc.exists) {
      const data = userDoc.data() || {};
      const userRole = String(data.role || data.staffRole || '').toLowerCase();
      if (data.isAdmin === true || ['superadmin', 'admin', 'clinical_lead', 'moderator', 'support'].includes(userRole)) {
        const isSuper = userRole === 'superadmin' || userRole === 'admin' || data.isAdmin === true;
        return { user, db, isSuperadmin: isSuper };
      }
    }
    ...
    if (bootstrapAdmin && user.email && user.email.toLowerCase() === bootstrapAdmin.toLowerCase()) {
      return { user, db, isSuperadmin: true };
    }
...
/**
 * Handler: List all staff members and privileged roles
 */
async function handleListStaff(req: VercelRequest, res: VercelResponse) {
  const authCtx = await requireAdminAuth(req, res);
  if (!authCtx) return;

  const { db, user } = authCtx;
  try {
    const staffMap = new Map<string, any>();

    // 1. Fetch all docs in /admins collection (index-safe, direct collection fetch)
    ...
    adminSnap.forEach((doc) => {
      ...
      if (data.isActive !== false) {
        staffMap.set(doc.id, {
          ...
          assignedByUid: data.assignedByUid || data.assignedBy || null,
          updatedAt: data.updatedAt || null,
        });
      }
    });

    // 2. Fetch all users marked with isAdmin: true (simple single-field query)
    const usersSnap = await db.collection('users').where('isAdmin', '==', true).get();
    usersSnap.forEach((doc) => {
      const data = doc.data() || {};
      const role = String(data.role || data.staffRole || '').toLowerCase();
      const existing = staffMap.get(doc.id) || {};
      staffMap.set(doc.id, {
        uid: doc.id,
        email: data.email || existing.email || '',
        displayName: data.displayName || existing.displayName || '',
        role: role || existing.role || 'superadmin',
        isAdmin: true,
        assignedAt: data.assignedAt || existing.assignedAt || null,
        assignedByUid: data.assignedByUid || existing.assignedByUid || null,
        updatedAt: data.updatedAt || existing.updatedAt || null,
      });
    });

    // Ensure the current authenticated administrator is in the staff directory
    if (!staffMap.has(user.uid)) {
    ...
    // Sort in-memory safely (superadmins first, then alphabetically)
    ...
    console.error('[API admin/list-staff] Error:', error);
    return res.status(500).json({ error: 'Internal Server Error', message: error.message || 'Failed to list staff.' });
  }
}

/**
 * Handler: Set staff role, custom claims, and record immutable audit log
 */
async function handleSetRole(req: VercelRequest, res: VercelResponse, parsedBody?: any) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });
  ...
  const { user: adminUser, db } = authCtx;
  const { auth: adminAuthInstance } = initFirebaseAdmin();
  const body = parsedBody || (await parseRequestBody(req));
  const { targetUid: explicitUid, newRole, targetEmail } = body || {};

  if (!newRole) {
    return res.status(400).json({ error: 'Bad Request', message: 'newRole is required.' });
  }

  const validRoles = ['superadmin', 'admin', 'clinical_lead', 'moderator', 'support', 'student'];
  if (!validRoles.includes(newRole)) {
    return res.status(400).json({ error: 'Bad Request', message: `Invalid role: ${newRole}. Allowed: ${validRoles.join(', ')}` });
  }

  let resolvedUid = explicitUid;
  let userRecordEmail = targetEmail;

  try {
    if (!resolvedUid && targetEmail) {
      const userRecord = await adminAuthInstance.getUserByEmail(targetEmail);
      resolvedUid = userRecord.uid;
      userRecordEmail = userRecord.email || targetEmail;
    } else if (resolvedUid && !userRecordEmail) {
      try {
        const userRecord = await adminAuthInstance.getUser(resolvedUid);
        userRecordEmail = userRecord.email;
      } catch {}
    }

    if (!resolvedUid) {
      return res.status(400).json({ error: 'Bad Request', message: 'Could not resolve target user UID or email.' });
    }

    const isStaffRole = newRole !== 'student';

    // 1. Mutate Firebase Auth custom claims
    try {
      await adminAuthInstance.setCustomUserClaims(resolvedUid, {
        role: newRole,
        admin: isStaffRole,
        isAdmin: isStaffRole,
        staffRole: newRole,
      ...
      console.warn('[handleSetRole] Failed setting custom claims via Firebase Auth:', claimErr);
    }
    ...
    if (isStaffRole) {
      ...
      updateData.betaAccessExpiresAt = null;
      updateData.isTrial = false;
      updateData.hasPaidAccess = true;
      updateData.paymentStatus = 'approved';
    }
    await userRef.set(updateData, { merge: true });

    // 3. Update /admins collection
    const adminRef = db.collection('admins').doc(resolvedUid);
    if (isStaffRole) {
      await adminRef.set({
        uid: resolvedUid,
        role: newRole,
        email: userRecordEmail || null,
        isActive: true,
        assignedByUid: adminUser.uid,
        assignedAt: FieldValue.serverTimestamp(),
        updatedAt: FieldValue.serverTimestamp(),
    ...
    // 4. Write to adminAuditLogs
    await logAdminAction(db, adminUser, 'set_staff_role', resolvedUid, {
      newRole,
      targetEmail: userRecordEmail,
      isStaffRole,
    });
    ...
    return res.status(500).json({ error: 'Internal Server Error', message: error.message || 'Failed to update staff role.' });
  }
}

/**
 * Handler: Direct Self-Elevation & Access Reconciliation Endpoint
 */
async function handleReconcileAdminAccess(req: VercelRequest, res: VercelResponse) {
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method Not Allowed' });

  const user = await requireAuth(req, res);
  if (!user) return;

  const { auth: adminAuthInstance, db } = initFirebaseAdmin();
  const callerUid = user.uid;

  try {
    // 1. Direct custom user claims synchronization
    try {
      await adminAuthInstance.setCustomUserClaims(callerUid, {
        role: 'superadmin',
        isAdmin: true,
        admin: true,
      ...
      console.warn('[reconcileAdminAccess] Claims error:', claimErr);
    }

    // 2. Synchronize users/{callerUid}
    ...
    // 3. Synchronize admins/{callerUid}
    ...
    await logAdminAction(db, user, 'reconcile_admin_access', callerUid, {
      email: user.email,
    });

    return res.status(200).json({
      success: true,
      message: 'Admin access reconciled and verified. All trial locks and paywall triggers cleared.',
      uid: callerUid,
      isAdmin: true,
      role: 'superadmin',
    });
  } catch (error: any) {
    console.error('[API admin/reconcile-admin-access] Error:', error);
    return res.status(500).json({ error: 'Internal Server Error', message: error.message || 'Failed to reconcile access.' });
  }
}

/**
 * Handler: List administrative audit logs
 */
async function handleListAuditLogs(req: VercelRequest, res: VercelResponse) {
  const authCtx = await requireAdminAuth(req, res);
  if (!authCtx) return;
  ...
  try {
    ...
    console.error('[API admin/list-audit-logs] Error:', error);
    return res.status(500).json({ error: 'Internal Server Error', message: error.message || 'Failed to list audit logs.' });
  }
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  switch (action) {
    ...
    case 'set-role':
      return handleSetRole(req, res, body);

    case 'reconcile-admin-access':
      return handleReconcileAdminAccess(req, res);
    ...
    default:
      return res.status(404).json({
        error: 'Not Found',
        message: `Unknown admin action: '${action}'. Supported actions: list-staff, set-role, reconcile-admin-access, list-audit-logs, approve-ambassador, reject-ambassador, update-access, bulk-update-access, update-affiliate, delete-user, review-payment, inspect-vault, health.`,
      });
  }
}
