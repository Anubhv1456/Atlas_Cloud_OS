...
import { UsmleAtlasSkyShareModal } from './UsmleAtlasSkyShareModal';
import { useExamProfile } from '@/hooks/useExamProfile';
import { isSubjectInProfScope, getPhaseNameForProfile } from '@/lib/curriculumScope';

interface UsmleAtlasSkyModalProps {
  open: boolean;
...
export type PhaseType = 'principles' | 'systems' | 'clinicals';
...
export function UsmleAtlasSkyModal({ open, onOpenChange, subjects, systems, curriculumSets }: UsmleAtlasSkyModalProps) {
  const [, setLocation] = useLocation();
  const [activeFilter, setActiveFilter] = useState<'all' | 'principles' | 'systems' | 'clinicals' | 'decay'>('all');
  ...
  const isStep1 = profile.targetExam?.toLowerCase().includes('step 1');
  const isStep2 = profile.targetExam?.toLowerCase().includes('step 2');
  
  const CELESTIAL_SUBJECTS = useMemo(() => {
    let config: Omit<CelestialSubject, 'x' | 'y'>[] = [];
    if (isStep2) {
      config = [
        { name: 'Internal Medicine', shortName: 'IM', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 0, radiusPercent: 35 },
        { name: 'Surgery', shortName: 'Surgery', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 60, radiusPercent: 35 },
        { name: 'Pediatrics', shortName: 'Peds', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 120, radiusPercent: 35 },
        { name: 'Obstetrics & Gynaecology', shortName: 'OB/GYN', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 180, radiusPercent: 35 },
        { name: 'Psychiatry', shortName: 'Psych', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 240, radiusPercent: 35 },
        { name: 'Family Medicine', shortName: 'FM', phase: 'clinicals', phaseLabel: 'Clinical Clerkship', angle: 300, radiusPercent: 35 }
      ];
    } else {
          ...
          <div className="relative flex-1 w-full flex items-center justify-center z-10 my-auto py-2 overflow-hidden">
            <div className="relative w-full aspect-square max-w-[min(100%,68vh,520px)] max-h-[min(100%,68vh,520px)] mx-auto flex items-center justify-center">
              ...
              <defs>
                ...
                <linearGradient id="goldenSynergy" x1="0%" y1="0%" x2="100%" y2="0%">
                 ...
                 return renderChain.map((currStar, index) => {
                    ...
                    return (
                      <g key={`constellation-line-${prevStar.name}-${currStar.name}`}>
                        ...
                        <circle cx={prevStar.x} cy={prevStar.y} r="0.75" fill="url(#starlightJointHalo)" opacity={isFiltered ? 0.9 : 0} />
                        <circle cx={currStar.x} cy={currStar.y} r="0.75" fill="url(#starlightJointHalo)" opacity={isFiltered ? 0.9 : 0} />
                      </g>
                    );
                 });
              })()}
              ...
              {mappedStars.map(star => {
                ...
                if (searchQuery.trim().length > 0) {
                  ...
                  if (activeFilter === 'principles') {
                    matchesFilter = star.phase === 'principles';
                  } else if (activeFilter === 'systems') {
                    matchesFilter = star.phase === 'systems';
                  } else if (activeFilter === 'clinicals') {
                    matchesFilter = star.phase === 'clinicals';
        ...
        {/* BOTTOM HUD INSPECTOR & FILTER BAR CONTAINER */}
          <div className="z-30 w-full max-w-xl mx-auto space-y-3">
            ...
            <div className="flex flex-wrap items-center justify-center gap-1.5 p-1.5 rounded-2xl bg-white/[0.02] border border-white/[0.08] backdrop-blur-md">
              ...
              <button
                onClick={() => setActiveFilter('principles')}
                className={cn(
                  "px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer",
              ...
              <button
                onClick={() => setActiveFilter('clinicals')}
                className={cn(
                  "px-3 py-1.5 rounded-xl text-xs font-medium transition-all cursor-pointer",
                  activeFilter === 'clinicals'
                    ? "bg-teal-500/20 text-teal-300 border border-teal-500/30 font-semibold"
                    : "text-zinc-400 hover:text-zinc-200 hover:bg-white/5"
                )}
              >
