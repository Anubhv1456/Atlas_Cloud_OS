...
export function SystemCard(props: SystemCardProps) {
  ...
  const { expanded, setExpanded, showRenameDialog, setShowRenameDialog, renameValue, setRenameValue, handleRenameSave, showDeleteConfirm, setShowDeleteConfirm, handleDeleteConfirm, showInsightDialog, setShowInsightDialog, insightType, setInsightType, insightContent, setInsightContent, insightSource, setInsightSource, isSubmittingInsight, handleInsightSubmit, showViewMarkersDialog, setShowViewMarkersDialog, selectedTopicId, setSelectedTopicId, selectedTopicName, setSelectedTopicName, showEvalDialog, handleEvalSelect, scoreModalTopicId, scoreModalTopicName, showScoreModal, setShowScoreModal, cardRef, progress, completedCount, contentPct, revisionDue, revisionOverdue, overdueDays, toggleQBank, localNotes, handleStatusChange, handleNotesChange, handleDelete, handleRevisionComplete, handleUpdateTopic, handleRenameTopic, handleDeleteTopic, handleAddCustomTopic, finalTopics, showLogSession, setShowLogSession, handleSetLogScore, showDecayCalibration, setShowDecayCalibration, toggleHighYield } = useSystemCardLogic(props);
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-3 p-4 pb-2">
          ...
          <button onClick={(e) => { e.stopPropagation(); toggleHighYield(); }} className="focus:outline-none"><Star className={cn("w-4 h-4 shrink-0 transition-colors", system.isHighYield ? "text-amber-500 fill-amber-500" : "text-muted-foreground/40 hover:text-amber-500/70")} /></button>
