          ...
          <div className="flex items-center gap-2 text-muted-foreground uppercase tracking-widest text-xs font-bold">
            ...
            {activeSubjectName && (
              <button
                ...
                className="text-xs lowercase font-normal px-2 py-0.5 rounded-full bg-muted/80 hover:bg-muted text-muted-foreground hover:text-foreground transition-all cursor-pointer"
              >
                (reset to all)
              </button>
            )}
        ...
        <div className={`border rounded-2xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-5 mb-8 transition-colors ${studyRecommendation.isCritical ? 'bg-rose-500/5 border-rose-500/20' : 'bg-primary/5 border-primary/20'}`}>
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap mb-1">
              ...
              <span className={`text-xs font-bold uppercase tracking-widest px-2.5 py-1 rounded border ${studyRecommendation.badgeColor}`}>
              ...
              {scoredSubjects.map(sub => {
                ...
                return (
                  ...
                  >
                    <span>{sub.name}</span>
                    <span className={cn(
                      "text-xs font-mono font-normal px-1 py-0.2 rounded",
