...
type QueueTab = 'action_needed' | 'support' | 'markers';
...
export function OpsQueueView() {
  ...
  const fetchAllData = async () => {
    setRefreshing(true);
    try {
      const [supportData, markersData, statsData] = await Promise.all([
        getContactMessages(),
        getAllMarkersForAdmin(),
        getDashboardStats()
      ]);
              ...
              {actionItemsCount === 0 ? (
                <div className="py-16 text-center border border-border/50 rounded-2xl bg-card/40 space-y-3">
                  ...
                  <p className="text-xs text-muted-foreground max-w-md mx-auto">
                    No unread support tickets, or reported community markers right now.
                  </p>
                </div>
              ) : (
